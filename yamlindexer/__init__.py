...
class YamlIndex():
    ...
    def __init__(self, root_path: str = None, globs: list = None, cache: Optional[bool] = None,
                 cache_ttl: Optional[int] = None, encoding: Optional[str] = None, loader: Optional[str] = None):
    ...
    def _index_files(self) -> dict:
        ...
        for p in files:
            with open(p, 'r', encoding=self.encoding) as f:
                try:
                    yamlf = yaml.load(f, Loader=getattr(yaml, f'C{self.yaml_loader}'))
                except AttributeError:
                    yamlf = yaml.load(f, Loader=getattr(yaml, self.yaml_loader))
